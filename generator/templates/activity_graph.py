"""GitHub contribution activity graph (area + line), built from the
contribution calendar fetched via GraphQL — self-hosted replacement for the
third-party github-readme-activity-graph widget, same hand-rolled SVG
approach as the rest of this generator.
"""

from generator.utils import resolve_theme, format_number, esc
...
def generate(config: dict, calendar: list, days: int = 31) -> str:
    ...
    parts = [
        ...
        f'<text x="24" y="30" class="title" font-family="\'JetBrains Mono\', monospace" font-size="14" font-weight="700">Contribution Activity — Last {days}d</text>',
    ]
    ...
    values = [p.get("count", 0) for p in points]
    total = sum(values)

    if len(points) == 1:
        parts.append(
            f'<text x="{width/2}" y="{height/2 - 6}" text-anchor="middle" class="title" '
            f'font-family="\'JetBrains Mono\', monospace" font-size="26" font-weight="700">{format_number(values[0])}</text>'
        )
        parts.append(
            f'<text x="{width/2}" y="{height/2 + 16}" text-anchor="middle" class="sub" '
            f'font-family="\'JetBrains Mono\', monospace" font-size="11">{esc(points[0]["date"])} · 데이터가 더 쌓이면 추이가 표시됩니다</text>'
        )
        parts.append("</svg>")
        return "\n".join(parts)
    ...
    parts.append(
        ...
        f'font-family="\'JetBrains Mono\', monospace" font-size="11">Total: {format_number(total)}</text>'
    )
